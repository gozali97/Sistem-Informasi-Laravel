<x-app-layout>
    ...
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Laporan /</span> Pendaapatan Laboratorium
    </h4>
                ...
                <table class="table table-striped" id="table1">
                    ...
                    @foreach ($data as $d)
                        <tr>
                            ...
                            @endphp
                            <td>{{ $umur }}</td>
                            <td>{{ date('d-m-Y', strtotime($d->kasir_tanggal)) }}</td>
                            <td>{{ $d->prsh_nama }}</td>
                            @php
                            ...
                            @endphp
                            <td>{{ $layanan }}</td>
                            <td>RP. {{ number_format($d->kasir_jumlah, 0, ',', '.') }}</td>
                        </tr>
        ...
        $(document).ready(function () {
            ...
            function updateTable(data) {
                ...
                $.each(data, function (index, d) {
                    ...
                    var formattedTanggal = new Date(d.kasir_tanggal).toLocaleDateString('en-GB');

                    var harga = 'RP.' + d.kasir_jumlah.toLocaleString('id-ID', {minimumFractionDigits: 0});
