<x-app-layout>
    ...
    @endif
    <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Master /</span>Tambah Promo Layanan Laboratorium
    </h4>
    ...
    <div class="card">
        ...
        <form id="myForm" action="{{ route('layananpromo.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="card-body">
                <div class="row">
                            ...
                            <select name="tarif_kelompok" class="form-control" id="tarif_kelompok" required>
                                ...
                                @foreach ($tarif as $t)
                                    <option value="{{ $t->var_kode }}"
                                        {{ old('tarif_kelompok') == $t->var_kode ? 'selected' : '' }}>
                                        {{ $t->var_nama }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-control" id="lab" name="tarif_kode">
                                ...
                                @foreach ($lab as $l)
                                    <option value="{{ $l->tarif_kode }}"
                                        {{ old('lab') == $l->tarif_kode ? 'selected' : '' }}>{{ $l->tarif_nama }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <div class="col-md-6">
                                <div>
                                    <label for="promo_start" class="form-label">Promo Start<sup
                                            class="text-danger">*</sup></label>
                                    <input type="date" id="promo_start" value="{{ old('promo_start') }}"
                                           name="promo_start" class="form-control" placeholder="dd/mm/yyyy" required>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div>
                                    <label for="promo_end" class="form-label">Promo End<sup
                                            class="text-danger">*</sup></label>
                                    <input type="date" id="promo_end" value="{{ old('promo_end') }}"
                                           name="promo_end" class="form-control" placeholder="dd/mm/yyyy" required>
                                </div>
                            </div>
                        </div>
                        <div class="mt-2">
                    ...
                    <div class="col-md-6 mb-3">
                        <div>
                            ...
                            <input type="number" id="tarif" value="{{ old('tarif_jalan') }}" name="tarif_jalan"
                        ...
                        <div class="mt-2">
                            <label for="persen_promo" class="form-label">Besar Promo %<sup
                                    class="text-danger">*</sup></label>
                            <input type="text" id="persenpromo" value="{{ old('persen_promo') }}"
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.length === 0) {
                            ...
                            $("#lab").append('<option>-- Pilih --</option>');
